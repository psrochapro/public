...
function sanitizeState() {
    ...
    ["ratio-16-9", "ratio-4-3", "ratio-1-1", "ratio-4-5", "ratio-9-16"].forEach(l => {
        if(!state.layoutSettings[l]) { state.layoutSettings[l] = JSON.parse(JSON.stringify(DEFAULT_TYPOGRAPHY[l])); }
        ...
        if (s.footer_m === undefined) s.footer_m = d.footer_m;
        if (s.mini_gap === undefined) s.mini_gap = d.mini_gap;
        if (s.mini_padding === undefined) s.mini_padding = d.mini_padding;
        if (s.mini_footer_gap === undefined) s.mini_footer_gap = d.mini_footer_gap;
    });
}
