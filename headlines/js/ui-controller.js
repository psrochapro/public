...
function setupSidebarInputs() {
    ...
    for (let i = 0; i < 3; i++) {
        handleImageUpload(`edit-thumb-${i}`, async (res) => { state.miniNoticias[i].thumb_url = await cropImage(res, 1/1); render(); });
    ...
    safeListener('edit-img-zoom', 'oninput', (e) => { state.noticiaPrincipal.zoom = e.target.value; document.documentElement.style.setProperty('--img-zoom', e.target.value); });
    safeListener('edit-img-y', 'oninput', (e) => { state.noticiaPrincipal.yPos = e.target.value; document.documentElement.style.setProperty('--img-y', e.target.value + '%'); });
    ...
    safeListener('edit-site-url', 'oninput', (e) => { state.config.site_url = e.target.value; const urlDisplay = document.getElementById('site-url-text'); if(urlDisplay) urlDisplay.innerText = e.target.value; });
    ...
    safeListener('edit-mini-footer-gap', 'oninput', updateSpacing('mini_footer_gap'));
...
function syncSidebarWithState() {
    ...
    setVal('edit-padding-y', s.padding_y); setVal('edit-padding-x', s.padding_x);
    setVal('edit-gap-card', s.gap_card); setVal('edit-footer-m', s.footer_m);
    setVal('edit-mini-gap', s.mini_gap); setVal('edit-mini-padding', s.mini_padding);
    setVal('edit-mini-footer-gap', s.mini_footer_gap);
    ...
    setVal('edit-main-cat', state.noticiaPrincipal.categoria); setVal('edit-main-date', state.noticiaPrincipal.data);
    setVal('edit-main-title', state.noticiaPrincipal.titulo); setVal('edit-main-sub', state.noticiaPrincipal.subtitulo);
    ...
    document.body.className = layout; syncTypographyUI();
}